-- Fix for ID type mismatch in vector search functions
-- Date: April 30, 2026

-- 1. Fix match_document_chunks_hybrid (Remove ::uuid cast for document_id)
...
BEGIN
  ...
  SELECT
    dc.id,
    dc.document_id, -- Removed ::uuid cast
    dc.version_id,
    dc.content,
    dc.metadata,
...
-- 2. Fix match_document_chunks (Remove ::uuid cast for document_id)
