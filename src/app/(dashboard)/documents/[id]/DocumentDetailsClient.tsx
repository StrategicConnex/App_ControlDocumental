...
export default function DocumentDetailsClient({
  ...
  const handlePrint = useReactToPrint({
    contentRef: printRef,
    documentTitle: `Documento_${doc.code}_${doc.title}`,
  });
    ...
    <div className="space-y-8 max-w-6xl mx-auto">
      <header className="flex items-center justify-between gap-4 border-b border-gray-100 pb-6">
        ...
        <div className="flex items-center gap-3">
          ...
          >
            <Printer size={16} /> Imprimir Detalles
          </button>
          {doc.file_url && (
            <a
              href={doc.file_url}
      ...
      <div ref={printRef} className="print:p-8">
        ...
        <div className="hidden print:block mb-8 border-b pb-4">
          <div className="flex justify-between items-center">
            <div>
              <h1 className="text-2xl font-bold text-slate-900">Ficha de Documento</h1>
              <p className="text-sm text-slate-500">Generado el: {new Date().toLocaleDateString()} {new Date().toLocaleTimeString()}</p>
            </div>
            <div className="text-right">
              <p className="font-bold text-slate-900">Strategic Connex</p>
              <p className="text-xs text-slate-500">DMS Enterprise Platform</p>
            </div>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div className="lg:col-span-2 space-y-8">
            ...
            <section className="bg-white p-6 rounded-[2rem] card-shadow border border-gray-100 print:hidden">
              <h3 className="font-bold text-gray-900 mb-6 flex items-center gap-2">
                      ...
                      >
                        ...
                        {!isCurrent && (
                          <button
                            ...
                            className="text-xs font-medium text-indigo-600 hover:text-indigo-700 transition-colors disabled:opacity-50 disabled:cursor-not-allowed print:hidden"
                          >
